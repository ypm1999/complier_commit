    ...
    public void visit(FuncIR node) {
        ...
        while (changed) {
            ...
            for (BasicBlockIR bb : BBList) {
                if (bb.getFronters().isEmpty() || (!basic && bb.getInstNum() > CJUMP_MAX_BB_INST_NUM))
                    continue;
                ...
                if (lastInst instanceof JumpInstIR) {
                    ...
                    if (instNum < (basic ? MAX_COPY_INST_NUM : CJUMP_MAX_COPY_INST_NUM)
                            && nextBB.getFronters().size() * instNum < MAX_TOTAL_COPY_INST_NUM) {
                        lastInst.remove();
                        bb.merge(nextBB.copy());
                        nextBB.getFronters().remove(bb);
                        changed = true;
                    }
                }
            }
